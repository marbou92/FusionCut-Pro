...
| Branches -> Add branch protection rule for `main` | Require status checks: `Format check`, `Core tests (ubuntu-latest)`, `Core tests (windows-latest)`, `Core tests (ubuntu, ASan+UBSan)`, `Media tests (ubuntu, FFmpeg 6.1)`, `Media tests (jammy container, FFmpeg 4.4)`, `Media tests (windows, MinGW64 FFmpeg)`, `Qt app build (Ubuntu, Qt 5.15)` | Keeps engine + media layer green on all compilers/API generations before merge. ci.yml runs 7 jobs; the core matrix reports as two check names, so 8 checks total are required |
