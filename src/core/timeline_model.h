...
struct Clip {
    ...
    // per-clip effect stack, applied top-to-bottom when the
    ...
    // text/title clips. A text clip has NO source (sourcePath
    ...
    // mutators keep all invariants (the text special-cases are:
...
struct Track {
    ...
    std::string name;
    bool isAudio = false;
    // text/title lanes. Video and audio clips cannot move to
...
// Frame-accurate timeline model (the editing core).
// Pure data + operations, no Qt, no FFmpeg - unit tested in fc_timeline_tests.
// Owned placement, split, trim, and move; adds cross-track moves
...
// adds cut transitions: every mutating operation preserves the
...
public:
    ...
    // adds a TEXT clip to a text track. The document is
    ...
    // durationFrames, rate 1.0 - all mutators keep working (see the
    ...
    // every text clip covering `frame`, in COMPOSITING order -
    ...
    // (project loading): the whole model is replaced in one
    ...
    // move a clip to another track and/or position. Rejects invalid
    ...
    // resolve the nearest valid drop start for a clip of the given
    ...
    // ripple delete: remove the clip and shift every later clip on the
    // same track left by exactly its duration, closing the gap it leaves.
    bool rippleDelete(int64_t id);

    // ripple trim end: trimClipEnd + shift every clip that started at
    ...
    // rolling edit: move the shared boundary of two adjacent clips on
    ...
    // the clip the program monitor should show at a timeline frame -
