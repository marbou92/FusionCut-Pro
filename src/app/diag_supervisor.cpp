...
// 2048 (not MAX_PATH=260) because the log path = parent dir (up to 1023)
// + ~38-char timestamped filename; 2048 is well above any realistic
// Windows path and satisfies -Wformat-truncation worst-case analysis.
char g_logPath[2048] = {0};
...
struct ExcInfo {
    ...
    unsigned long long avTarget; // the faulting VA (AV only)
    ...
    // Module containing the AV TARGET address - for loader-phase faults
    // this names the DLL whose (invalid) data the loader was reading,
    // which is the actual culprit (the instruction blame lands in
    // ntdll, which is only the messenger). Empty if the target lies
...
bool alreadyVisited(const char *name) {
    for (int i = 0; i < g_visitedCount; ++i) {
        ...
        // keeps the "only kernel32 + user32" constraint clean).
...
HANDLE openLog(const char *selfDir) {
    SYSTEMTIME st;
    GetLocalTime(&st);
    snprintf(g_logPath, sizeof(g_logPath), "%s\\FusionCutPro-diag-%04d%02d%02d-%02d%02d%02d.log",
    ...
    snprintf(g_logPath, sizeof(g_logPath), "%sFusionCutPro-diag-%04d%02d%02d-%02d%02d%02d.log", tmp,
...
// - so a bad DllMain in a dependency cannot crash this process. If the
...
void walkImports(HMODULE h, const char *contextName, int depth) {
    ...
    for (; imp->Name != 0; ++imp) {
        ...
        // DllMain crashes (phase 2 covers those).
...
void recordException(const DEBUG_EVENT &ev) {
    ...
    // Attribute the AV TARGET address too. For a loader-phase fault the
    // instruction address blames ntdll (the loader code), but the DATA
    // being read belongs to the module named here - that module is the
    // real culprit.
...
void runWatch(const char *selfPath, const char *workDir, WatchResult &res) {
    ...
    // The child is THIS exe with the --diag-child marker: main.cpp
    // starts the normal app path for it (no recursion into --diag).
    char cmd[1100];
    snprintf(cmd, sizeof(cmd), "\"%s\" --diag-child", selfPath);

    if (!CreateProcessA(selfPath, cmd, NULL, NULL, FALSE,
    ...
    for (;;) {
        ...
        if (!WaitForDebugEvent(&ev, 2000)) {
            ...
            if (sawInitialBreakpoint && elapsed > 25000) {
                ...
                // declare healthy and stop the child before it shows a
        ...
        case EXCEPTION_DEBUG_EVENT: {
            ...
            if (code == 0x80000003 && !sawInitialBreakpoint) {
                ...
                if (secondChance) {
                    ...
                    // child so no WER dialog pops on the user's screen.
...
void logWatch(const WatchResult &res) {
    ...
    logLine(line);
    for (int i = 0; i < g_modCount; ++i) {
        // %.*s with the precision of the name field: gcc models the
        // worst-case string length of g_mods[i].name across the whole
        // g_mods array (up to the array end), which trips -Wformat-
        // truncation on an unbounded %s; the explicit precision is the
        // honest bound (names are NUL-terminated well before 1023).
        snprintf(line, sizeof(line), "  [%3d] base=0x%016llx  size=0x%08llx  %.*s", i + 1,
                 g_mods[i].base, g_mods[i].size, (int)(sizeof(g_mods[i].name) - 1), g_mods[i].name);
        logLine(line);
    }
    logLine("");
...
int fc::runDiagSupervisor(const std::string &appVersion) {
    // Derive the portable folder from this exe's own path; the child
    // instance is the SAME exe (self-debugging, see the header).
    char selfPath[1024];
    const DWORD n = GetModuleFileNameA(NULL, selfPath, sizeof(selfPath) - 1);
    if (n == 0 || n >= sizeof(selfPath) - 1) {
        MessageBoxA(NULL, "FusionCut Pro --diag: cannot resolve the executable path.",
                    "FusionCut Pro - Diagnostic", MB_OK | MB_ICONERROR);
        return 1;
    }
    selfPath[n] = 0;
    char selfDir[1024];
    ...
    if (last) {
        *last = 0;
        ...
        GetCurrentDirectoryA(sizeof(selfDir) - 1, selfDir);
        selfDir[sizeof(selfDir) - 1] = 0;
    }
    ...
    logLine("==============================================================");
    logLine("FusionCut Pro diagnostic supervisor (baked-in loader check)");
    {
        // 2112 = sizeof(g_logPath) + 64 slack for the fixed prefixes.
        char hdr[2112];
        snprintf(hdr, sizeof(hdr), "Version: %s", appVersion.c_str());
        logLine(hdr);
        snprintf(hdr, sizeof(hdr), "Target (self): %s", selfPath);
        logLine(hdr);
        snprintf(hdr, sizeof(hdr), "Log: %s", g_logPath);
        logLine(hdr);
    }
    logLine("==============================================================");
    logLine("");
    ...
    // ----- phase 1 -----
    HMODULE h = LoadLibraryExA(selfPath, NULL, DONT_RESOLVE_DLL_REFERENCES);
    if (!h) {
        // The target itself can't be mapped - e.g. the file is corrupt
        // or truncated mid-extract.
        logFail("FusionCutPro.exe", GetLastError(), "(self)");
    } else {
        markVisited("FusionCutPro.exe");
        walkImports(h, "FusionCutPro.exe", 0);
        FreeLibrary(h); // refcount-symmetric probe (see walkImports note)
    }

    // ----- phase 2 -----
    WatchResult res;
    runWatch(selfPath, selfDir, res);
    logWatch(res);
    ...
    if (res.outcome == 0 && res.fatalIdx >= 0) {
        ...
        snprintf(summary, sizeof(summary),
                 "Could not spawn the child instance (err=%lu).\r\nPhase 1: %d import "
                 "failure(s).\r\n\r\n"
                 "Full details in the log:\r\n%s",
    ...
    MessageBoxA(NULL, summary, "FusionCut Pro - Diagnostic (--diag)",
...
int fc::runDiagSupervisor(const std::string &appVersion) {
    std::fprintf(stderr,
                 "FusionCut Pro %s: --diag is the Windows loader diagnostic "
                 "(import-tree probe + debug-launch watch); there is nothing "
