...
LONG WINAPI vectoredExceptionHandler(PEXCEPTION_POINTERS ep) {
    ...
    const std::string dir = g_reportDir.empty() ? exeDirectory() : g_reportDir;
    const std::string path = joinPath(dir, "FusionCutPro-crash-" + timestamp() + ".log");
    FILE *fp = std::fopen(path.c_str(), "wb");
    if (!fp) {
        // Fall back to current directory; report what we tried.
        std::fprintf(stderr, "FusionCut Pro: cannot open crash log at %s; writing to cwd\n",
                     path.c_str());
        const std::string fallback = "FusionCutPro-crash-" + timestamp() + ".log";
        fp = std::fopen(fallback.c_str(), "wb");
        if (fp) {
    ...
    std::snprintf(msg, sizeof(msg),
                  ...
                  ep->ExceptionRecord->ExceptionAddress, path.c_str());
...
#if defined(_WIN32)
    // VEH first: catches all SEH exceptions in all threads, including the
    // qwindows.dll platform-plugin init path. FirstHandler=1 means we
    // run *before* any other VEH registered later by Qt/MinGW runtime.
    AddVectoredExceptionHandler(1, vectoredExceptionHandler);
    _set_invalid_parameter_handler(invalidParameterHandler);
    _set_purecall_handler(pureCallHandler);
#else
    // POSIX: pre-open a log fd so the signal handler can write without
    // touching libc malloc. Opened once here; reused per-crash.
    const std::string dir = g_reportDir.empty() ? exeDirectory() : g_reportDir;
    const std::string path = joinPath(dir, "FusionCutPro-crash-" + timestamp() + ".log");
